...
void ElfImg::openElf(const char* path) {
    // load elf
    int fd =
        open(path,
             O_RDONLY | O_CLOEXEC);  // Pine changed: add O_CLOEXEC to flags
    if (fd == -1) {
        return;
    }
...
Elf_Addr ElfImg::getSymbOffset(const char* name) const {
    ...
    if (dynsym_start != nullptr && strtab_start != nullptr) {
        ...
        for (k = 0; k < dynsym_count; k++, sym++) {
            char* current = strings + sym->st_name;
...
void* ElfImg::getSymbAddress(const char* name) const {
